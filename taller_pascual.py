...
# ==========================================
# 2. LÓGICA DE CORREO, DIRECTORIOS Y NUBE
# ==========================================
def enviar_correo(destinatario, asunto, mensaje_texto, pdf_bytes, nombre_archivo, email_real_local):
    try:
        if "email" in st.secrets:
            ...
            return False, "Faltan credenciales del sistema de correos en Secrets (st.secrets['email'])."
...
def cargar_directorio_correos():
    ...
    default_c = [{"Nombre": "Gabriel Poblete", "Email": "sistema.cotizador.gp@gmail.com"}]
    if client:
        try:
...
def detectar_cliente_automatico(patente_input):
    ...
    if not match.empty:
        ...
        if "GENDARMERÍA" in inst or "GENDARMERIA" in inst: return "GENDARMERÍA DE CHILE", "Gendarmería de Chile"
        elif "SAMU" in inst: return "SAMU", "SAMU"
        elif "TEMUCO" in inst: return inst, "Hospital Temuco"
        elif "VILLARRICA" in inst: return inst, "Hospital Villarrica"
        elif "LAUTARO" in inst: return inst, "Hospital Lautaro"
        elif "PITRUFQUEN" in inst or "PITRUFQUÉN" in inst: return inst, "Hospital Pitrufquén"
        else: return inst, "SSAS (Servicio Salud)"
    return None, None
...
# ==========================================
# 3. BASE DE DATOS DE VEHÍCULOS (CSV DINÁMICO)
# ==========================================
...
# ==========================================
# 4. RUT Y CORRELATIVOS (MODO EMPRESA)
# ==========================================
...
# ==========================================
# 5. DATOS DE LA EMPRESA Y ESTILOS
# ==========================================
...
# ==========================================
# 8. UI PRINCIPAL MODO WHATSAPP (UNA PÁGINA)
# ==========================================
...
with col_centro[1]:
    ...
    with tab1:
        ...
        # Cambio: "Costado Fijo" ahora es "Ventana Lateral"
        ...
        if st.button("➕ Agregar Producto al Resumen", use_container_width=True):
            for p in productos_temp:
                if p['desc'] and p['precio'] > 0:
                    st.session_state.items_productos.append({"Descripción": p['desc'], "Cantidad": p['cant'], "Unitario": p['precio'], "Total": p['precio'] * p['cant']})
    ...
    if total_bruto > 0:
        ...
        # --- SECCIÓN 4: PDF FORMAL (ESCONDIDO) ---
        st.markdown("---")
        with st.expander("🏢 Habilitar Cotización Formal (PDF para Empresas / Seguros)", expanded=False):
            ...
            for c in clientes_db:
                ...
                if rut_str not in clientes_dict: clientes_dict[rut_str] = {'Nombre': c['Nombre'], 'RUT': c['RUT'], 'Direccion': c['Direccion'], 'Ciudad': c['Ciudad'], 'Comuna': c['Comuna'], 'Giro': c['Giro'], 'Contactos': []}
            ...
            if sel_cli != "--- Nuevo Cliente ---":
                ...
                if cli_data:
                    def_nombre = str(cli_data.get('Nombre', '')); def_rut = str(cli_data.get('RUT', '')); def_dir = str(cli_data.get('Direccion', '')); def_ciu = str(cli_data.get('Ciudad', '')); def_com = str(cli_data.get('Comuna', '')); def_giro = str(cli_data.get('Giro', ''))
            ...
            if 'presupuesto_generado' not in st.session_state:
                ...
                if not data.get('respaldo_ok'): st.warning(f"⚠️ Error al respaldar en la nube: {data.get('respaldo_msj')}")
